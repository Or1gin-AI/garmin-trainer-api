...
const runTempo: WorkoutTemplate = {
  ...
  fixed: {
    ...
    contraindications: ['latestStimulus.threshold', 'latestStimulus.vo2max', 'latestStimulus.anaerobic', 'hardSessions.atCap'],
    downgradeTo: 'run.aerobic.v1',
...
const runInterval: WorkoutTemplate = {
  ...
  variables: {
    ...
    cooldownDuration: { source: { kind: 'template_default', default: 13, unit: 'minutes' } },
    mainDurationTotal: {
      source: { kind: 'derived', from: 'intervalRepeats,intervalDistance,intervalPace,recoveryDuration', rule: 'sum of fast segments at intervalPace + (n-1) * recovery', unit: 'minutes' },
    },
...
const runProgression: WorkoutTemplate = {
  ...
  fixed: {
    ...
    phases: [
      ...
      { name: 'main', label: '第三段 Tempo 下沿', duration: '$segmentDuration', description: 'RUN.tempoPace 下沿，HR.tempo (不进入 HR.threshold)。' },
      { name: 'cooldown', label: '放松', duration: '$cooldownDuration' },
    ],
    ...
    minDurationMinutes: 50,
    maxDurationMinutes: 75,
    notes: '天气炎热或坡度大配速不可控时禁用；第三段不提到 Tempo，只做到标准有氧。',
  },
  variables: {
    warmupDuration: { source: { kind: 'template_default', default: 10, unit: 'minutes' } },
    ...
    tempoPace: { source: { kind: 'athlete_profile', path: 'athleteProfile.running.tempoPaceSecPerKm', unit: 's/km' } },
...
const runRacePace: WorkoutTemplate = {
  ...
  variables: {
    ...
    cooldownDuration: { source: { kind: 'template_default', default: 13, unit: 'minutes' } },
    mainDurationTotal: {
      source: { kind: 'derived', from: 'raceRepeats,raceIntervalDistance,racePace,recoveryDuration', rule: 'raceRepeats * raceIntervalDistance / racePace + (n-1) * recoveryDuration', unit: 'minutes' },
    },
