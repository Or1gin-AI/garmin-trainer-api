...
- **AI access gate** lives in `api/src/lib/quota.ts`. `requireAiPlanAndQuota(kind)` now only checks that the user has active Max AI access; it no longer rejects Max users by monthly generation/chat counts. `consumeQuota(...)` still increments `ai_usage` after successful operations so `/admin → 用量` remains useful for monitoring token and request volume.
...
- **Usage accounting**: `requireProAndQuota(kind)` is kept as an alias for the Max-only access gate. `consumeQuota(userId, kind, …)` increments usage only after the SSE `done` event is emitted (i.e. after the persistence transaction commits). Failed generations / chats do not increment. Usage increment errors are logged (`console.error`) but never propagated to the client — the user already received their successful response.
